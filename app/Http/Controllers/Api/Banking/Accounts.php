...
use App\Jobs\Banking\CreatePatient;
use App\Jobs\Banking\DeletePatient;
use App\Jobs\Banking\UpdatePatient;
...
class Accounts extends ApiController
{
    ...
    {
        $account = $this->dispatch(new CreatePatient($request));
    ...
    {
        try {
            $account = $this->dispatch(new UpdatePatient($account, $request));
    ...
    {
        $account = $this->dispatch(new UpdatePatient($account, request()->merge(['enabled' => 1])));
    ...
    {
        try {
            $account = $this->dispatch(new UpdatePatient($account, request()->merge(['enabled' => 0])));
    ...
    {
        try {
            $this->dispatch(new DeletePatient($account));
